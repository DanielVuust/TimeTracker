using TimeRegistration.TimeTracker.ApplicationServices.Repositories;
...
namespace TimeRegistration.TimeTracker.Infrastructure.BaseRepository;
public abstract class BaseRepository<T, TE> : IBaseRepository<T> where T : BaseModel where TE : BaseEntity
{
    ...
    protected BaseRepository(TimeTrackerContext context)
    {
        Context = context;
